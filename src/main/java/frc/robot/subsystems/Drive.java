...
//Remeber that the CANcoders are absolute encoders used to determine offset of angle motor only
//Below relative encoder has 2 ports, for chA and chB
//For WPILib "Encoder" class - not absolute encoder, requires 2 channels, plus direction indicated here
//TODO - Mr. Bersing will want to use external encoders for the drive shaft for accuracy ??
//getDistance, getRate 
//for now, try to use the internal TalonFX encoder (sensor) - without need for Encoders below
              ...
              log -> {
                ...
                log.motor("drive-left")
                   ...
                   .linearPosition(m_distance.mut_replace(m_leftEncoder.getDistance(), Meters)) //distance since last reset, as scaled by setDistancePerPuls
                    .linearVelocity(
                        m_velocity.mut_replace(m_leftEncoder.getRate(), MetersPerSecond));  //distance per second, as scaled by the value of distance per pulse
